 ...
 */
word_t crc_bitwise(model_t *, word_t, unsigned char *, size_t);
...
/* Equivalent to crc_bitwise(), but use a faster byte-wise table-based
   ...
   crc_table_bytewise(). */
word_t crc_bytewise(model_t *, word_t, unsigned char *, size_t);
...
word_t crc_wordwise(model_t *, word_t, unsigned char *, size_t);
